  ...
  _attachTLSListeners() {
    ...
    this.socksProxy.alpnCache.get(rewriteToLocalhostIfNeeded(this.host), this.port, alpnProtocolChosenByServer => {
      ...
      dummyServer.on('secureConnection', internalTLS => {
        ...
        if (!_net.default.isIP(this.host)) tlsOptions.servername = this.host;
        if (process.env.PWTEST_UNSUPPORTED_CUSTOM_CA && (0, _utils.isUnderTest)()) tlsOptions.ca = [_fs.default.readFileSync(process.env.PWTEST_UNSUPPORTED_CUSTOM_CA)];
        const targetTLS = _tls.default.connect(tlsOptions);
        ...
        targetTLS.on('error', error => {
          _debugLogger.debugLogger.log('client-certificates', `error when connecting to target: ${error.message}`);
