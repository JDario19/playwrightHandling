...
class TestServerDispatcher {
  ...
  async _collectingInternalReporter() {
    const {
      collectingReporter,
      report
    } = await this._collectingReporter();
    return {
      reporter: new _internalReporter.InternalReporter(collectingReporter),
      report
    };
  }
  ...
  async runGlobalSetup(params) {
    ...
    const listReporter = new _list.default();
    const taskRunner = (0, _tasks.createTaskRunnerForWatchSetup)(config, [collectingReporter, listReporter]);
    ...
    await taskRunner.reporter.onEnd({
      status
    });
  ...
  async listFiles(params) {
    ...
    const taskRunner = (0, _tasks.createTaskRunnerForListFiles)(config, [collectingReporter]);
    taskRunner.reporter.onConfigure(config.config);
    const testRun = new _tasks.TestRun(config);
  ...
  async _innerListTests(params) {
    ...
    const taskRunner = (0, _tasks.createTaskRunnerForList)(config, [collectingReporter], 'out-of-process', {
    ...
    if (this._watchTestDirs) await this.updateWatcher(false);
    return {
      report,
      status
    };
  }
  async updateWatcher(reportPending) {
  ...
  async _innerRunTests(params) {
    ...
    const taskRunner = (0, _tasks.createTaskRunnerForTestServer)(config, reporters);
    const testRun = new _tasks.TestRun(config);
    taskRunner.reporter.onConfigure(config.config);
